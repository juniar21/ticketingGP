...
const StasYear = () => {
  ...
  useEffect(() => {
    if (orders.length > 0) {
      ...
      orders.forEach((order) => {
        ...
        const year = date.getFullYear().toString(); // Hanya Tahun

        if (!grouped[year]) {
          grouped[year] = 0;
        }
        grouped[year] += order.amount;
      });
  ...
  return (
    <div className="w-full h-[500px]">
      <h2 className="text-2xl font-bold mb-4">Orders Chart (by Year)</h2>
